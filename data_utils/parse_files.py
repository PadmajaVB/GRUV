...
def convert_np_audio_to_sample_blocks(song_np, block_size):  #this returns song_np by padding it
    ...
    print('song_np.shape[0]=', total_samples)
    ...
    while (num_samples_so_far < total_samples):
        ...
        if (block.shape[0] < block_size):
            ...
            block = np.concatenate((block, padding))  # block_size is 11025 which is fixed throughout whereas block.shape[0] for the last block is <=11025
...
def time_blocks_to_fft_blocks(blocks_time_domain):
    ...
    for block in blocks_time_domain:
        ...
        new_block = np.concatenate((np.real(fft_block), np.imag(fft_block)))  #Joins a sequence of arrays along an existing axis.
...
def convert_wav_files_to_nptensor(directory, block_size, max_seq_len, out_file, max_files=20, useTimeDomain=False):
    ...
    print('len(chunks_X)=',len(chunks_X))
...
def load_training_example(filename, block_size=2048, useTimeDomain=False):
    ...
    x_t = convert_np_audio_to_sample_blocks(data, block_size) #x_t has the padded data i.e with 0's in the empty space of the last block
    y_t = x_t[1:]
